...
se_samples = [sample for sample in samplesheet if samplesheet[sample]['end_type'] == 'se']
pe_samples = [sample for sample in samplesheet if samplesheet[sample]['end_type'] == 'pe']
...
def get_bamfilter_params(wildcards):
    if metadata_df.loc[(wildcards.sample), "end_type"] == "se":
        return " -F {remove} -q {mapqual} ".format(remove=config['filter_bam']['se_remove_reads_flags'],mapqual=
        config['filter_bam']['mapqual_filter'])
    elif metadata_df.loc[(wildcards.sample), "end_type"] == "pe":
        return " -F {remove} -f {keep} -q {mapqual} ".format(remove=config['filter_bam']['pe_remove_reads_flags'],keep=
        config['filter_bam']['pe_keep_reads_flags'],mapqual=config['filter_bam']['mapqual_filter'])
